...
run_gfx950_bwd_v3() {
    ...
    for v3_bf16_cvt in 0 1 2 ; do
    for mask in 0 ; do
    for batch in 1280 ; do
    for head in 2 ; do
    for seq in 256 512 ; do

    $EXE -prec=$prec -b=$batch -h=$head -d=128 -s=$seq -mask=$mask -bwd_v3=1 -v3_atomic_fp32=$v3_atomic_fp32 -v3_bf16_cvt=$v3_bf16_cvt -mode=0 -kname=$KNAME $COMMON_ARGS
